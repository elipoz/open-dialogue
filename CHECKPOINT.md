...
- **Moderator name gate:** Before any conversation, the moderator must enter their name (short text box, max 15 characters). All fields are hidden until a non-empty name is submitted. Caption: **"State your name as the moderator to start."** (larger, solid black). No Tavily line on this page. Page title becomes **"&lt;name&gt;'s Open Dialogue with AI"** once set. Conversation history and export use the **original author name** for each message (from DB when loaded); new messages use current user name.
...
- **Multi-user sync:** Multiple users can share the same conversation. **1s fragment** (right column) polls the current conversation and refreshes messages from the DB without full-page rerun (no input loss or blink). **10s fragment** (sidebar) refreshes the conversation list so new/deleted conversations appear within ~10s. New conversation is prepended to the list so it shows immediately for the creator. `conversation_exists()` in `supabase_client`; opening or polling a deleted conversation clears state and reloads. **Dedupe** by conversation id so only one row can be current; refetch stores deduped list; if current conversation is missing from the list (deleted by another user), selection is cleared and URL updated. Session-state reset consolidated in `_clear_conversation_state()`.
- **Timestamps in PST:** All UI timestamps (conversation list, message history) are shown in **America/Los_Angeles (PST)**. `_format_in_pst()` treats naive datetimes as UTC (e.g. from Supabase); new conversation label uses `_now_pst()`.
