...
- **@mentions:** `@g` / `@j` (first letter of agent name, case-insensitive, optional space after `@`) trigger that agent’s response; uses the same thinking spinner as the Respond button.
- **Layout:** 50% left (controls + spinner), 50% right (conversation history).
- **Conversation history:** Right panel, newest first by default; **Reverse order** toggles to chronological; timestamps on messages.
