...
- **Moderator name gate:** Before any conversation, the moderator must enter their name. All fields are hidden until a non-empty name is submitted. Conversation history and export show that name instead of "Moderator".
...
- **Layout:** Sidebar (wider, min 360px) with conversations list + 50% left (controls + spinner), 50% right (conversation history). **Reverse order** button on the same line as "Conversation history".
- **Conversation history:** Right panel, newest first by default; **Reverse order** toggles; timestamps. **Export to doc** downloads full conversation as .docx (via `doc_export.py`).
