...
- **OpenAI context:** Conversation sent to OpenAI as **"At &lt;timestamp&gt; &lt;role&gt; said: &lt;message&gt;"** so who said what and when is clear. Timestamp is message `created_at` from DB (always present); role uses actual names (moderator = user’s name, agents = display names e.g. Gosha, Joshi).
