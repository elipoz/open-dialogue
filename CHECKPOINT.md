...
- **Layout:** Sidebar with conversations list, divider, **Participants** expander, **Request / response log**. Main: 50% left (controls + spinner), 50% right (conversation history). Above "Conversation history" subheader: **Reverse order** (left) and **Export to doc** (right) when there is dialogue.
